...
const onAddQuestion = (type) => {
    ...
    $('#problem').append(ele)
    problem.push({problemName: '', mustAnswer: true, option: [{}], isTwo: 0})
    relativeProblem.push({optionIndex: 0 , relativeOption: [ ]})
...
const onInputRelative = (problemIndex, optionIndex, relativeOptionIndex,key) => {
    problem[problemIndex].isTwo = 1
    relativeProblem[problemIndex].optionIndex = optionIndex
    relativeProblem[problemIndex].relativeOption[relativeOptionIndex] = $(`#question${problemIndex} #relativeOptionItem${relativeOptionIndex} #${key}`)[0].value
}
      ...
      <div class="bottom">
        ...
        <div class="option" id="option">
          <div class="option-item" id="optionItem0">
            ...
            <br>
            <span class="relativeOption-add" onclick="addRrelativOption(${relativeProblem.length})">添加关联选项</span>
          </div>
        </div>
...
const addRrelativOption = (problemIndex) => {

    $(`#question${problemIndex} #option`).append(`
    <div class="relativeOption-item" id="relativeOptionItem${relativeProblem[problemIndex].relativeOption.length}">
      <input type="text" class="form-control" id="relativeTerm" placeholder="关联选项" oninput="onInputRelative(${problemIndex}, ${problem[problemIndex].option.length},${relativeProblem[problemIndex].relativeOption.length}, 'relativeTerm')" />
      <span class="option-del" onclick="delRrelativOption(${problemIndex}, ${relativeProblem[problemIndex].relativeOption.length})">删除</span>
    </div>
...
const delRrelativOption = (problemIndex, optionIndex) => {
    $(`#question${problemIndex} .relativeOption-item`)[optionIndex].remove()
    relativeProblem[problemIndex].relativeOption.splice(optionIndex, 1)
    $(`#question${problemIndex} .relativeOption-add`).map((item, index) => {
        index.onclick = delRrelativOption.bind(this, problemIndex, item)
    })
}
...
const importQuestion = () => {
        location.href = "/pages/designQuestionnaire/modules/importquestion/index.html"
}

const handleModifyTitle = () => {
    const questionnaireTitle = $util.getItem('surveyName')
    const questionnaireDescription = $util.getItem('surveyDescription')
...
onload = () => {
        ...
        const flag = $util.getItem('flag')

        console.log(flag)
        if(flag>0){
            const questionnaireInput = $util.getItem('questionnaireInput')
            console.log(questionnaireInput)

            const option1 = $util.getItem('option1')
            const option2 = $util.getItem('option2')
            const option3 = $util.getItem('option3')
            handleAddSingleChoice();
            $('#question0#problemName').val(questionnaireInput)
            $('#question0#optionItem0#chooseTerm').val(option1)
            $('#question0#optionItem1#chooseTerm').val(option2)
            $('#question0#optionItem2#chooseTerm').val(option3)
        }
...
const handleEditFinish = () => {
    ...
    for (var i = 0; i < problem.length; i++) {
            ...
            let params = {
                ...
                questionName: problem[i].problemName,
                questionDesc: problem[i].mustAnswer,
                firstOption: problem[i].option[0].chooseTerm,
                secondOption: problem[i].option[1].chooseTerm,
                thirdOption: problem[i].option[2].chooseTerm,
                isTwo: problem[i].isTwo
            }


        $.ajax({
            url: API_BASE_URL + '/creatQuestion',
            type: "POST",
            ...
            success(res) {
                if (params.isTwo == 1){
                    ...
                    let relative = {
                            questionId:res.data,
                            optionIndex:relativeProblem[0].optionIndex,
                        relativeFirstOption:relativeProblem[0].relativeOption[0],
                        relativeSecondOption:relativeProblem[0].relativeOption[1],
                        relativeThirdOption:relativeProblem[0].relativeOption[2],
                    }
                        $.ajax({
                            url: API_BASE_URL + '/creatSecondOption',
                            type: "POST",
                            data: JSON.stringify(relative),
                            dataType: "json",
                            contentType: "application/json",
                            success(res) {
                                console.log(res)
